...
Take a look at the bottom of the following script and modify the paths to point to your CCD definitions.
...
Parameters:
- `roles`: a comma-separated list of roles. Roles must be existing IDAM roles for the CCD domain. Every caseworker requires at least its coarse-grained jurisdiction role (`caseworker-<jurisdiction>`).
